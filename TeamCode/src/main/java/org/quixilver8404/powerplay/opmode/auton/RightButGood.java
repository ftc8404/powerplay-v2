...
public class RightButGood extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        robot.poseModule.setStartPos(new Vector3(17.5/2 * 0.0254,(141 - 100) * 0.0254,0));

        if (isStopRequested()) {
            robot.stopHardwareLoop();
            return;
        }
        robot.pidPositionEstimation.setPoint(new Vector3(posX*0.0254,(141 - 100) * 0.0254,0));
...
//        robot.pidPositionEstimation.goY();
        robot.pidPositionEstimation.setPoint(new Vector3(posX*0.0254,(141 - 100) * 0.0254,-Math.PI/2));
        robot.pidPositionEstimation.goTheta();
        while (!robot.pidPositionEstimation.isNotMoving() && opModeIsActive()){
            telemetry();
        }
